<x-backend.layouts.master>
    @php
         foreach ($yearwisestudents as $yearwisestudent)
            {
                $year = $yearwisestudent->year;
                $course_year = $yearwisestudent->course_year;
            }
    @endphp
    <x-slot name='breadCrumb'>
        <x-backend.layouts.elements.breadcrumb>
            <x-slot name="pageHeader"> {{ $course_year ??' ' }} year, {{ $year ?? ' ' }} Result Upload  </x-slot>

            <li class="breadcrumb-item"><a href="{{ route('year.students')}}">Dashboard</a></li>
    ...
    <script>
        ...
        // {
        //     const fetch_url = `http://127.0.0.1/api/admin/get-courses/${course_year}/${year}/${student_id}`;
        //     alert(fetch_url);
